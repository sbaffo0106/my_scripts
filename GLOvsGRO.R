...
library(xtable)
#
# define functions 20 BINS
analytic_codist0 <- function(x){ #EXPONENTIAL FINAL LP
 a=1.20483274
 b=2.41901608
 as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
}
analytic_codist1 <- function(x){ #EXPONENTIAL FINALsf LP
 a=1.16305515
 b=2.58691212
 as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
}
analytic_codist2 <- function(x){ #EXPONENTIAL FINALp LP
 a=1.0546817
 b=3.53108493
 as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
}
# analytic_codist0 <- function(x){ #EXPONENTIAL FINALgg LP (mhalo: 1.14996887 2.81757595)
#  a=1.16448138
#  b=2.72178178
#  as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
# }
# analytic_codist1 <- function(x){ #EXPONENTIAL FINALggsf LP (mhalo: 1.13332368 2.81270622)
#  a=1.13528901
#  b=2.79359142
#  as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
# }
# analytic_codist2 <- function(x){ #EXPONENTIAL FINALggp LP (mhalo: 1.0756764 3.39372449)
#  a=1.07558079
#  b=3.39285364
#  as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
# }
# analytic_codist0 <- function(x){ #EXPONENTIAL FINALug LP
#   a=1.25009928
#   b=2.09144154
#   as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
# }
# analytic_codist1 <- function(x){ #EXPONENTIAL FINALugsf LP
#   a=1.19455533
#   b=2.35436036
#   as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
# }
# analytic_codist2 <- function(x){ #EXPONENTIAL FINALugp LP
#   a=1.04267416
#   b=3.60794145
#   as.numeric(x>=8)*pmin(867.0,exp((x-b)/a))
# }
...
analytic_vol1 <- function(x){
  totarea1 = (54.93 + 57.44 + 56.93) #* 2.1816555113301354e-05
...
analytic_vol2 <- function(x){
  totarea2 = (54.93 + 57.44 + 56.93) #* 2.1816555113301354e-05
...
analytic_vol3 <- function(x){ 
  totarea3 = (54.93 + 57.44 + 56.93) #* 2.1816555113301354e-05
...
# Start of main code
#
stub="/Users/antonio/Desktop/ANTONIO/PAPER2/GLOvsGRO/"
#
# Read in catalogue
morphcat1=fread(paste0(stub,"FINAL_LP.csv"))
morphcat2=fread(paste0(stub,"FINALstarforming_LP.csv"))
morphcat3=fread(paste0(stub,"FINALpassive_LP.csv"))
#
...
#DoubleSchechterPENG <- function(x, p) {
#  mu = (10^(x-10.8372890))
#  log(10) * exp(-mu) * (10^p[1] * (mu)^(p[2]+1) + 10^p[3] * mu^(p[2]))
#}
#SingleSchechterPENG <- function(x, p) {
#  mu = (10^(x-10.8372890))
#  log(10) * exp(-mu) * (10^p[1] * (mu)^(-1.4496318+1))
#}
#
rho=1.0
...
Omega2 = totarea2 / 3282.806  #[deg2 to sr] FOV
Omega3 = totarea3 / 3282.806  #[deg2 to sr] FOV
dVdr1 = function(r) r^2*Omega1*rho
dVdr2 = function(r) r^2*Omega2*rho
dVdr3 = function(r) r^2*Omega3*rho
#
#morphx=morphcat[redshift <= 0.213 & ppp >= 0.01] # PPP
...
#
pinitt = c(log10(16.27e-4), 10.97, -0.53, log10(9.47e-4), -1.37) #no Vcorr
##pinitt = c(1, 10.97, -0.53, 1, -1.37) #si Vcorr
...
# pblue <- c(-3.258780, 10.796996, -1.164509, -4.067154, -1.164509) #grouped (alpha1=alpha2)
...
# Plot GSMF
#
png(filename=paste0(stub,"prova1_INPUT.png"),width=22.0,height=15.0,units="cm",pointsize = 18,res=240)
#
...
abline(v = 10^10.48, col = "blue", lwd = 1, lty = 1)
abline(v = 10^10.74, col = "purple", lwd = 1, lty = 1)
dev.off()
#
...
legend("topright", 
       legend = "(a) All galaxies", 
       bty = "n",   # no box
       cex = 1)   # dimensione del testo
#
dev.off()
###################################
#1. SS & DS
...
#
#
png("/Users/antonio/Desktop/ANTONIO/PAPER2/GLOvsGRO/veff_DS.png", width = 800, height = 600, res = 120)
dfplotveff(survey2b, xlab = expression(M), ylab = expression(V[eff]~"[Mpc"^3~"]"))
dev.off()
png("/Users/antonio/Desktop/ANTONIO/PAPER2/GLOvsGRO/veff_SS.png", width = 800, height = 600, res = 120)
dfplotveff(survey2a, xlab = expression(M), ylab = expression(V[eff]~"[Mpc"^3~"]"))
dev.off()
#